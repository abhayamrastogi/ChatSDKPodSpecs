...
- (NSString *)uploadFile:(NSString *)methodName parameters:(NSArray *)parameters responseCallback:(MeteorClientMethodCallback)responseCallback{
   ...
   [_methodIds addObject:methodId];
    ...
    [self.ddp uploadFileWithId:methodId method:methodName parameters:parameters];
...
- (NSString *)_send:(BOOL)notify parameters:(NSArray *)parameters methodName:(NSString *)methodName {
    ...
    NSString *methodId;
    if ([methodName isEqualToString:@"ufsWrite"]){
        methodId = @"ufsWrite iOS";
    }else{
     methodId = [BSONIdGenerator generate];
    }
    if(notify == YES) {
        [_methodIds addObject:methodId];
    }
